...
class Auth():
    ...
    def get_reset_password_token(self, email: str) -> str:
        ...
        If it exists generate a UUID and update the user reset
        token database field.

        Args:
            email (str): email of the user whose reset token
            to update

        Returns:
            str: the user reset token
        """
        try:
            user = self._db.find_user_by(email=email)
        except NoResultFound:
    ...
    def update_password(self, reset_token: str, password: str) -> None:
        """updates the user password and reset token field

        Args:
            reset_token (str): reset token of the user to update
            password (str): new password of the user to update to
        """
        try:
